...
const base='http://127.0.0.1:8756', out='release/amenities-20260909';
(async()=>{const b=await chromium.launch({headless:true});const reports=[];
for(const width of [320,390,768,1440]){
 const p=await b.newPage({viewport:{width,height:1000},reducedMotion:'reduce'}),errors=[],providerErrors=[];
 p.on('pageerror',e=>{const trace=e.stack||e.message;if(trace.includes('https://app.lodgify.com/'))providerErrors.push(trace);else errors.push(trace);});
 ...
 const metrics=await p.evaluate(()=>({height:document.body.scrollHeight,overflow:document.documentElement.scrollWidth>innerWidth,proofTop:Math.round(document.querySelector('.guest-proof').getBoundingClientRect().top+scrollY),badImages:[...document.images].filter(x=>x.getAttribute('src')&&(!x.complete||!x.naturalWidth)).map(x=>x.src),bathrooms:document.querySelectorAll('.bathroom-card').length,bedrooms:document.querySelectorAll('.room-card').length,features:document.querySelectorAll('.homepage-blog-card').length,allUpstairs:document.querySelector('#rooms').textContent.includes('All four bedrooms and both full bathrooms are upstairs.'),h1:document.querySelectorAll('h1').length,canonical:document.querySelector('link[rel=canonical]').href,overlaps:[...document.querySelectorAll('.homepage-blog-card')].some(x=>x.querySelector('.homepage-blog-photo').getBoundingClientRect().right>x.querySelector('h3').getBoundingClientRect().left+2&&innerWidth<=700)}));
 assert.equal(metrics.overflow,false);assert.deepEqual(metrics.badImages,[]);assert.equal(metrics.bathrooms,0);assert.equal(metrics.bedrooms,0);assert.equal(metrics.features,3);assert.equal(metrics.h1,1);assert(metrics.allUpstairs);assert.equal(metrics.overlaps,false);
 ...
 for(const id of ['living','photo-4','photo-40','coffee','karaoke','court','yard']){
 ...
 for(const selector of ['#backyard','#gallery','#rooms']) { if([390,1440].includes(width)) await p.locator(selector).screenshot({style:'.site-header, .mobile-booking, .skip-link { visibility: hidden !important; }',path:out+'/'+selector.slice(1)+'-'+width+'.png'}); }
 await p.evaluate(()=>scrollTo(0,0));await p.screenshot({path:out+'/home-final-'+width+'.png',fullPage:true});
 assert.deepEqual(errors,[]);reports.push({page:'home',width,...metrics,menu_gallery_booking_film_accordions:'passed',provider_entry:provider,providerErrors,errors});console.log(JSON.stringify({width,height:metrics.height,proofTop:metrics.proofTop,state:'passed'}));
 await p.close();
}
